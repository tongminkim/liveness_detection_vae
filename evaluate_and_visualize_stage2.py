...
# Output directory
OUTPUT_DIR = "runs/stage2_evaluation"
...
def get_fake_types(data_split, split_name='final_test'):
    """Extract fake types from file paths"""
    fake_files = data_split[split_name]['fake']
    fake_types = []
    for f in fake_files:
        if 'audio-driven' in f:
            fake_types.append('audio-driven')
        elif 'dfl' in f:
            fake_types.append('dfl')
        elif 'dffs' in f:
            fake_types.append('dffs')
        else:
            fake_types.append('unknown')
...
def plot_1_reconstruction_loss_distribution(results_m1, results_m2, output_path):
    ...
    ax.hist(results_m1['scores'][fake_mask_m1], bins=50, alpha=0.6, label='Fake', color='red', density=True)
    ...
    ax.hist(results_m2['scores'][fake_mask_m2], bins=50, alpha=0.6, label='Fake', color='red', density=True)
...
def plot_5_score_by_fake_type(results_m1, results_m2, fake_types, output_path):
    ...
    for fake_type in ['audio-driven', 'dfl', 'dffs']:
        ...
        if mask.sum() > 0:
            ...
            ax.hist(scores, bins=30, alpha=0.6, label=f'{fake_type} (n={mask.sum()})', density=True)
...
def main():
    ...
    # Get fake types
    fake_types = get_fake_types(data_split, 'final_test')
    ...
    plot_5_score_by_fake_type(
        results_m1, results_m2, fake_types,
