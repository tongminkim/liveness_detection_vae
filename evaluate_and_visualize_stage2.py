"""
Stage 2 Methods 평가 및 시각화
- Method 1 (Margin Loss) vs Method 2 (Discriminator)
- Final test set (data_split.json의 'final_test') 사용
- 5가지 시각화:
...
DEFAULT_OUTPUT_DIR = "runs/stage2_evaluation"
AUDIODRIVEN_OUTPUT_DIR = "runs/stage2_evaluation_audiodriven"
...
def determine_fake_type(file_path):
    """Infer fake type string from file path"""
    if "audio-driven" in file_path:
        return "audio-driven"
    if "dfl" in file_path:
        return "dfl"
    if "dffs" in file_path:
        return "dffs"
    return "unknown"


def get_fake_types_from_dataset(dataset):
    """Collect fake types for the current dataset ordering"""
    fake_types = []
    for path, label in zip(dataset.files, dataset.labels):
        if label == 1:
            fake_types.append(determine_fake_type(path))
    return np.array(fake_types)


def filter_audiodriven_samples(dataset):
    """
    Keep all real samples and only audio-driven fake samples.
    Returns True if filtering was applied.
    """
    keep_indices = []
    for idx, (path, label) in enumerate(zip(dataset.files, dataset.labels)):
        if label == 0 or "audio-driven" in path:
            keep_indices.append(idx)

    if len(keep_indices) == len(dataset.files):
        return False

    dataset.files = [dataset.files[i] for i in keep_indices]
...
def plot_1_reconstruction_loss_distribution(results_m1, results_m2, output_path):
    ...
    ax.hist(
        results_m1["scores"][real_mask_m1],
        bins=50,
        alpha=0.6,
        label="Real",
        color="blue",
    ...
    ax.hist(
        results_m1["scores"][fake_mask_m1],
        bins=50,
        alpha=0.6,
        label="Fake",
        color="red",
    ...
    ax.hist(
        results_m2["scores"][real_mask_m2],
        bins=50,
        alpha=0.6,
        label="Real",
        color="blue",
    ...
    ax.hist(
        results_m2["scores"][fake_mask_m2],
        bins=50,
        alpha=0.6,
        label="Fake",
        color="red",
...
def plot_2_roc_curve(results_m1, results_m2, output_path):
    ...
    ax.plot(
        fpr_m1, tpr_m1, label=f"Method 1 (Margin Loss) - AUC={auc_m1:.4f}", linewidth=2
    )
    ax.plot(
        fpr_m2,
        tpr_m2,
        label=f"Method 2 (Discriminator) - AUC={auc_m2:.4f}",
        linewidth=2,
    )
...
def plot_3_latent_2d_tsne(results_m1, results_m2, output_path):
    ...
    latent_combined_m1 = np.concatenate(
        [results_m1["latent_lf"], results_m1["latent_hf"]], axis=1
    )
    ...
    ax.scatter(
        latent_2d_m1[real_mask_m1, 0],
        latent_2d_m1[real_mask_m1, 1],
        c="blue",
        alpha=0.5,
        s=10,
        label="Real",
    ...
    ax.scatter(
        latent_2d_m1[fake_mask_m1, 0],
        latent_2d_m1[fake_mask_m1, 1],
        c="red",
        alpha=0.5,
        s=10,
        label="Fake",
    ...
    latent_combined_m2 = np.concatenate(
        [results_m2["latent_lf"], results_m2["latent_hf"]], axis=1
    )
    ...
    ax.scatter(
        latent_2d_m2[real_mask_m2, 0],
        latent_2d_m2[real_mask_m2, 1],
        c="blue",
        alpha=0.5,
        s=10,
        label="Real",
    ...
    ax.scatter(
        latent_2d_m2[fake_mask_m2, 0],
        latent_2d_m2[fake_mask_m2, 1],
        c="red",
        alpha=0.5,
        s=10,
        label="Fake",
...
def plot_4_latent_3d_tsne(results_m1, results_m2, output_path):
    ...
    latent_combined_m1 = np.concatenate(
        [results_m1["latent_lf"], results_m1["latent_bp"], results_m1["latent_hf"]],
        axis=1,
    )
    ...
    ax.scatter(
        latent_3d_m1[real_mask_m1, 0],
        latent_3d_m1[real_mask_m1, 1],
        latent_3d_m1[real_mask_m1, 2],
        c="blue",
        alpha=0.5,
        s=10,
        label="Real",
    ...
    ax.scatter(
        latent_3d_m1[fake_mask_m1, 0],
        latent_3d_m1[fake_mask_m1, 1],
        latent_3d_m1[fake_mask_m1, 2],
        c="red",
        alpha=0.5,
        s=10,
        label="Fake",
    ...
    latent_combined_m2 = np.concatenate(
        [results_m2["latent_lf"], results_m2["latent_bp"], results_m2["latent_hf"]],
        axis=1,
    )
    ...
    ax.scatter(
        latent_3d_m2[real_mask_m2, 0],
        latent_3d_m2[real_mask_m2, 1],
        latent_3d_m2[real_mask_m2, 2],
        c="blue",
        alpha=0.5,
        s=10,
        label="Real",
    ...
    ax.scatter(
        latent_3d_m2[fake_mask_m2, 0],
        latent_3d_m2[fake_mask_m2, 1],
        latent_3d_m2[fake_mask_m2, 2],
        c="red",
        alpha=0.5,
        s=10,
        label="Fake",
...
def plot_5_score_by_fake_type(results_m1, results_m2, fake_types, output_path):
    """
    Plot 5: Score Distribution by Fake Type
    """
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    ...
    fake_scores_m1 = results_m1["scores"][fake_mask_m1]
    fake_scores_m2 = results_m2["scores"][fake_mask_m2]

    # Method 1
    ax = axes[0]
    for fake_type in ["audio-driven", "dfl", "dffs"]:
        mask = fake_types == fake_type
        if mask.sum() > 0:
            scores = fake_scores_m1[mask]
            ax.hist(
                scores,
                bins=30,
                alpha=0.6,
                label=f"{fake_type} (n={mask.sum()})",
                density=True,
            )

    ax.set_xlabel("Reconstruction Loss")
    ax.set_ylabel("Density")
    ax.set_title("Method 1 (Margin Loss): Score by Fake Type")
    ax.legend()
    ax.grid(True, alpha=0.3)
    ...
    ax.set_xlabel("Discriminator Score")
    ax.set_ylabel("Density")
    ax.set_title("Method 2 (Discriminator): Score by Fake Type")
    ax.legend()
    ax.grid(True, alpha=0.3)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Stage 2 evaluation/visualization")
    parser.add_argument(
        "--audiodriven-only",
        action="store_true",
        help="Keep all real samples but restrict fake samples to audio-driven only",
    )
    parser.add_argument(
        "--output-dir",
        default=None,
        help="Optional custom output directory (defaults depend on --audiodriven-only)",
    )
...
def main(args):
    print("=" * 80)
    print("Stage 2 Methods Evaluation and Visualization")
    print("=" * 80)
    print()
    ...
    num_real = len(data_split["final_test"]["real"])
    num_fake = len(data_split["final_test"]["fake"])
    print(f"Final test set: {num_real} real, {num_fake} fake")
    print()
    ...
    if args.output_dir:
        ...
        output_dir = (
            AUDIODRIVEN_OUTPUT_DIR if args.audiodriven_only else DEFAULT_OUTPUT_DIR
        )
    ...
    if args.audiodriven_only:
        filtered = filter_audiodriven_samples(test_dataset)
        if filtered:
            real_count = sum(label == 0 for label in test_dataset.labels)
            fake_count = sum(label == 1 for label in test_dataset.labels)
            print(
                f"Filtered to audio-driven fake samples only: {real_count} real, {fake_count} fake"
            )
        else:
            print(
                "Audio-driven filter requested; no filtering applied (all fake samples already audio-driven)."
            )
    ...
    checkpoint_m1 = torch.load(
        METHOD1_CHECKPOINT, map_location=device, weights_only=False
    )
    ...
    print(
        f"  Mean score (Real): {results_m1['scores'][results_m1['labels'] == 0].mean():.4f}"
    )
    print(
        f"  Mean score (Fake): {results_m1['scores'][results_m1['labels'] == 1].mean():.4f}"
    )
    print()
    ...
    checkpoint_m2 = torch.load(
        METHOD2_CHECKPOINT, map_location=device, weights_only=False
    )
    ...
    discriminator_m2 = LatentDiscriminator(
        C_z=config.C_z, num_bands=3, hidden_dim=128
    ).to(device)
    ...
    print(
        f"  Mean score (Real): {results_m2['scores'][results_m2['labels'] == 0].mean():.4f}"
    )
    print(
        f"  Mean score (Fake): {results_m2['scores'][results_m2['labels'] == 1].mean():.4f}"
    )
    print()

    # Get fake types
    fake_types = get_fake_types_from_dataset(test_dataset)
    ...
    plot_5_score_by_fake_type(
        results_m1,
        results_m2,
        fake_types,
        os.path.join(output_dir, "5_score_by_fake_type.png"),
    )
    ...
    print(
        f"  - Separation: {results_m1['scores'][results_m1['labels'] == 1].mean() - results_m1['scores'][results_m1['labels'] == 0].mean():.4f}"
    )
    print()
    ...
    print(
        f"  - Separation: {results_m2['scores'][results_m2['labels'] == 1].mean() - results_m2['scores'][results_m2['labels'] == 0].mean():.4f}"
    )
    print()
