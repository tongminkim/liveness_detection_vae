"""
Evaluate Stage2 Method1 Margin Model on Final Test Set
Using Two-Stage Gaussian Filtering with PCA_10D

Model: stage2_method1_margin/stage2_method1_best.pt
...
MODEL_CHECKPOINT = '/home/elicer/liveness_detection/model1/runs/stage2_method1_margin/stage2_method1_best.pt'
OUTPUT_DIR = "runs/stage2_method1_margin_evaluation"
...
def plot_results(results, output_dir):
    ...
    title += f'Accuracy: {results["accuracy"]:.4f}, F1-Score: {results["f1"]:.4f}'
    ...
    ax2.set_title('Performance Metrics\n(Stage2 Method1 Margin)', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'stage2_method1_margin_results.png'), dpi=150, bbox_inches='tight')
    plt.close()
    print(f"Saved visualization to: {os.path.join(output_dir, 'stage2_method1_margin_results.png')}")
...
def main():
    print("="*80)
    print("Evaluating Stage2 Method1 Margin Model on Final Test Set")
    print("="*80)
    ...
    print(f"  Stage 1 pass rate: {results['stage1_pass_rate']:.2%} ({results['stage1_pass_count']}/{len(test_labels)})")
    print(f"  Stage 2 pass rate: {results['stage2_pass_rate']:.2%} ({results['stage2_pass_count']}/{len(test_labels)})")
    print(f"  Classified as Real: {np.sum(results['predictions'] == 0)}/{len(test_labels)} ({100*np.sum(results['predictions'] == 0)/len(test_labels):.2f}%)")
    print(f"  Classified as Fake: {np.sum(results['predictions'] == 1)}/{len(test_labels)} ({100*np.sum(results['predictions'] == 1)/len(test_labels):.2f}%)")
    print()
    ...
    metrics = {
        'model_name': 'stage2_method1_margin',
