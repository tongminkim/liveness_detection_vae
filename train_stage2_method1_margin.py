...
- Includes proper batch-wise tensor handling in train/validate loops
"""
...
def main():
    ...
    print(
        f"  lr: {config.lr}, epochs: {config.epochs}, batch_size: {config.batch_size}"
    )
    print("=" * 80)
    print()
    ...
    print(
        f"Loaded checkpoint from epoch {checkpoint['epoch']} (val_loss={checkpoint['val_loss']:.4f})"
    )
    print()
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        train_loss = train_epoch(
            model, train_loader, optimizer, device, MARGIN, use_amp=True
        )
