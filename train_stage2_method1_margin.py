"""
Stage 2: Method 1 - Margin Loss (Contrastive Learning)
...
def validate(model, loader, device, margin):
    ...
    for x_lf, x_bp, x_hf, labels in loader:
        ...
        for i in range(batch_size):
            ...
            if labels[i] == 0:  # Real
                real_rec_loss += loss_dict['total']
                n_real += 1
            else:  # Fake
                fake_rec_loss += loss_dict['total']
                n_fake += 1
    ...
    # Compute margin loss
    total_loss = margin_loss(real_rec_loss, fake_rec_loss, margin)

    return {
        'total': total_loss.item() if isinstance(total_loss, torch.Tensor) else total_loss,
        'real_rec': real_rec_loss.item() if isinstance(real_rec_loss, torch.Tensor) else real_rec_loss,
        'fake_rec': fake_rec_loss.item() if isinstance(fake_rec_loss, torch.Tensor) else fake_rec_loss,
        'separation': (fake_rec_loss - real_rec_loss).item() if isinstance(fake_rec_loss, torch.Tensor) else (fake_rec_loss - real_rec_loss)
    }
...
def train_epoch(model, loader, optimizer, device, margin, use_amp=True):
    ...
    for x_lf, x_bp, x_hf, labels in loader:
        ...
        if use_amp and torch.cuda.is_available():
            with torch.amp.autocast('cuda'):
                ...
                if real_mask.sum() > 0:
                    ...
                    real_loss = torch.zeros(1, device=device, requires_grad=True)
                ...
                if fake_mask.sum() > 0:
                    ...
                    fake_loss = torch.zeros(1, device=device, requires_grad=True)
        ...
        if isinstance(real_loss_dict['total'], torch.Tensor):
            real_rec_loss += real_loss_dict['total'].item()
        else:
            real_rec_loss += real_loss_dict['total']
        if isinstance(fake_loss_dict['total'], torch.Tensor):
            fake_rec_loss += fake_loss_dict['total'].item()
        else:
            fake_rec_loss += fake_loss_dict['total']
        n_real += real_mask.sum().item()
        n_fake += fake_mask.sum().item()

    return {
        'total': total_loss / n_batches,
        'real_rec': real_rec_loss / n_batches,
        'fake_rec': fake_rec_loss / n_batches,
        'separation': (fake_rec_loss - real_rec_loss) / n_batches
    }
...
def main():
    print("="*80)
    print("Stage 2: Method 1 - Margin Loss (Contrastive Learning)")
    print("="*80)
