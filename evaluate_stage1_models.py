"""
Evaluate Stage1 Pretrained Models on Final Test Set

Models:
1. stage1_pretrain/stage1_pretrained.pt - with adjusted thresholds (0.90, 0.75)
2. runs/stage1_pretrained.pt - with standard thresholds (0.95, 0.70)
...
# Two models to evaluate with their specific thresholds
MODELS = {
    'stage1_pretrain': {
        'path': '/home/elicer/liveness_detection/model1/runs/stage1_pretrain/stage1_pretrained.pt',
        'conf_stage1': 0.90,
        'conf_stage2': 0.75
    },
    'stage1_root': {
        'path': '/home/elicer/liveness_detection/model1/runs/stage1_pretrained.pt',
        'conf_stage1': 0.95,
        'conf_stage2': 0.70
    }
}

OUTPUT_DIR = "runs/stage1_models_evaluation"
...
def evaluate_model(model_name, model_info, train_loader, test_loader, test_labels, device):
    """Evaluate a single model"""
    print("\n" + "="*80)
    print(f"Evaluating: {model_name}")
    print("="*80)

    model_checkpoint = model_info['path']
    conf_stage1 = model_info['conf_stage1']
    conf_stage2 = model_info['conf_stage2']

    print(f"Loading model from: {model_checkpoint}")
    print(f"Thresholds: Stage1={conf_stage1}, Stage2={conf_stage2}")
    ...
    results = two_stage_filtering_pca10d(
        ...
        loss_mu_3d, loss_cov_3d,
        latent_mu_pca, latent_cov_pca,
        conf_stage1, conf_stage2
    )

    # Add PCA explained variance to results
    results['explained_variance'] = explained_var
    results['conf_stage1'] = conf_stage1
    results['conf_stage2'] = conf_stage2
...
def plot_comparison(results_dict, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'stage1_models_comparison.png'), dpi=150, bbox_inches='tight')
    plt.close()
    print(f"Saved comparison plot to: {os.path.join(output_dir, 'stage1_models_comparison.png')}")
...
def main():
    print("="*80)
    print("Evaluating Stage1 Pretrained Models on Final Test Set")
    print("="*80)
    ...
    for name, info in MODELS.items():
        ...
        print(f"    Thresholds: Stage1={info['conf_stage1']}, Stage2={info['conf_stage2']}")
    ...
    print("\n{:<25} {:<25} {:<25}".format("Metric", "stage1_pretrain", "stage1_root"))
    print("-"*80)
    print("{:<25} {:<25.4f} {:<25.4f}".format("Accuracy",
        results_dict['stage1_pretrain']['accuracy'],
        results_dict['stage1_root']['accuracy']))
    print("{:<25} {:<25.4f} {:<25.4f}".format("Precision",
        results_dict['stage1_pretrain']['precision'],
        results_dict['stage1_root']['precision']))
    print("{:<25} {:<25.4f} {:<25.4f}".format("Recall",
        results_dict['stage1_pretrain']['recall'],
        results_dict['stage1_root']['recall']))
    print("{:<25} {:<25.4f} {:<25.4f}".format("F1-Score",
        results_dict['stage1_pretrain']['f1'],
        results_dict['stage1_root']['f1']))
    print("{:<25} {:<25.4f} {:<25.4f}".format("Specificity",
        results_dict['stage1_pretrain']['specificity'],
        results_dict['stage1_root']['specificity']))
    print("{:<25} {:<25.4f} {:<25.4f}".format("Explained Variance",
        results_dict['stage1_pretrain']['explained_variance'],
        results_dict['stage1_root']['explained_variance']))
    print("-"*80)
    ...
    for model_name, results in results_dict.items():
        metrics = {
            ...
            'model_checkpoint': MODELS[model_name]['path'],
            'method': 'Two-Stage Gaussian Filtering (PCA_10D)',
            'pca_dim': PCA_DIM,
            'explained_variance': float(results['explained_variance']),
            'confidence_stage1': results['conf_stage1'],
            'confidence_stage2': results['conf_stage2'],
